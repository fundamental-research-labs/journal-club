...
% 讲述背景->发现现象 -> 举例描述现象，突出以往方法中存在的现象，仅体现在有限的场景下，并未呈现一个普适性。一个有价值的问题出现了，这个现象是否普遍的存在？
...
%However, these methods are implemented by dedicated prompt engineering \citep{spp} or complex multi-agent communication framework \citep{autoagent}. Is there a method which can enhance the LLM performance in a simple way?

% In these works, multiple LLM agents are instantiated to improve the performance of LLMs. For instance, LLM-Debate \cite{debate} employs multiple LLM agents in a debate form. The reasoning performance is improved by instantiating more agents in solving arithmetic tasks compared to instantiating a single agent. Similarly, CoT-SC \citep{cotsc} generates multiple thought chains, which can be implemented by multiple agents, and select the most consistent one as the final answer. The reasoning performance is improved by involving more thought chains compared to CoT \citep{cot} which employs a single thought chain. We find that the performance can be improved by scaling up the number of agents instantiated in these works. However, this phenomenon has been observed only in limited scenarios, and a comprehensive study is lacking. 

% Although large language models (LLMs) demonstrate remarkable capabilities in a variety of applications \citep{llmsurvey}, such as language generation, understanding, and reasoning, they struggle to provide accurate answers when faced with complicated tasks. To improve the performance of LLMs, some recent studies focus on deploying multiple LLM agents with ensemble methods \citep{cotsc, fusellm} or collaboration frameworks \citep{debate,autogen}. In these works, they typically claim that involving more agents in their proposed methodologies can yield better performance. However, is this phenomenon universal?

% To validate this phenomenon, we turn to the simplest sampling-and-voting method to leverage LLM agents. In Figure \ref{fig:intro_finding}, we find that, maybe surprisingly, the ensemble of smaller LLMs can achieve comparable or even superior performance compared with larger LLMs. We thus conduct a comprehensive study on how the performance scales with the ensemble size. 



% In these works, multiple LLM agents are instantiated to improve the performance of LLMs. For instance

%We note that these methods alprompt engineering \cite{iclsurvey} and multiple LLM-agents collaboration frameworks \cite{agentsurvey,agentsurvey1}. Despite the  implementations of these methods, some of these approaches exhibit the same phenomenon: the performance of LLM scales with the number of agents (LLMs) instantiated. For instance, LLM-Debate \cite{debate} employs multiple LLM agents in a debate form to enhance reasoning performance, and it is noted that the more agents involved, the greater the performance gains in arithmetic scenarios. Similarly, if we consider an LLM as an agent, CoT-SC \cite{cotsc} applies the Chain of Thought (CoT) approach \cite{cot} to multiple agents and notes improved performance with more agents. However, this phenomenon have been noted in only limited scenarios, and a comprehensive study is still lacking. An intriguing question arises: Does this phenomenon generally exist?


% Large language models (LLMs) demonstrate remarkable capabilities in language generation, understanding, and reasoning. Recent studys focus on unsupervised methods to further enhance LLM performance, such as prompt engineering \cite{iclsurvey} and multiple LLM-agents collaboration frameworks \cite{agentsurvey,agentsurvey1}. Despite the differing implementations of these methods, some of these approaches exhibit the same phenomenon: the performance of LLM scales with the number of agents (LLMs) instantiated. For instance, CoT-SC \cite{cotsc} samples from a single LLM by various Chain of Thought (CoT) \cite{cot} observes that involving more samples correlates with higher performance. Similarly, LLM-Debate \cite{debate} employs multiple LLM agents to enhance reasoning performance, and it is noted that the more agents involved, the greater the performance gains in arithmetic scenarios. However, these studies only exhibit the phenomenon in a few limited scenarios, and as of now, there has been no comprehensive study on this phenomenon. An intriguing question arises: Does this phenomenon generally exist?

%\textcolor{red}{I suggest you guys write the intro in Chinese using simple and clear language and ask GPT to translate. 1. a bit background. 2. what is motivating. 3. what did we do. 4. how we did it. 5. to sum up.}

Although large language models (LLMs) demonstrate remarkable capabilities in various applications \citep{llmsurvey}, such as language generation, understanding, and reasoning, they struggle to provide accurate answers when faced with complicated tasks. To improve the performance of LLMs, some recent studies focus on ensemble methods \citep{cotsc, fusellm} and multiple LLM-Agents collaboration frameworks \citep{debate,autogen}. However, these methods are implemented by dedicated prompt engineering \citep{spp} or complex multi-agent communication framework \citep{autoagent}. 
% \textcolor{red}{Is there a method that can enhance the LLM performance simply? are you serious... cannot express like this.... }


Interestingly, we have identified a phenomenon within these studies. Utilizing multiple LLM agents can enhance the effectiveness of LLMs. For instance, LLM-Debate \cite{debate} employs multiple LLM agents in a debate form. The reasoning performance is improved by instantiating more agents in solving arithmetic tasks compared to instantiating a single agent. Similarly, CoT-SC \citep{cotsc} generates multiple thought chains by multiple agents and selects the most consistent one as the final answer. The reasoning performance is improved by involving more thought chains compared to CoT \cite{cot} which employs a single thought chain. In these works, we find that the performance can be improved by scaling up the number of agents instantiated. An intriguing question arises: Can we generally enhance LLM performance by leveraging multiple LLM agents in a simple way?


% However, this phenomenon has been observed only in limited scenarios, and a comprehensive study is lacking. 
To address this question, we propose a simple sampling-and-voting method to leverage multiple LLM agents and conduct the first comprehensive study to verify its generalizability. Compared to existing studies \cite{cotsc,complexity-cot,debate} that claim the presence of this phenomenon, our study not only encompasses more tasks, including reasoning and generation, but also explores orthogonality with other methods. 

The result indicates that LLM performance can generally be improved by increasing the ensemble size (i.e., the number of agents). Surprisingly, Figure \ref{fig:intro_finding} shows that ensembles of smaller LLMs can sometimes achieve comparable or superior performance to larger LLMs. Moreover, by combining our method with other existing methods, we find the performance can be further improved. By comparing with the performance of complicated methods, the result shows that employing our method solely can achieve comparable performance in most cases. This implies that comparable performance can be achieved without the need for additional handcraft prompt design or complex collaboration frameworks.

% 我们首次围绕这个现象做了广泛的研究。
% We conduct the first comprehensive study on this phenomenon via a simple sampling-and-voting method which unfolds in two phases. Initially, the query is iteratively fed into a single LLM or a multiple LLM-Agents collaboration framework to generate multiple outputs. Subsequently, majority voting is used to determine the final result. The experiments are conducted by using various LLMs of different sizes on diverse datasets including reasoning tasks and code generation tasks. The result indicates that LLM performance can generally be improved by increasing the ensemble size (i.e., the number of agents). Surprisingly, ensembles of smaller LLMs can sometimes achieve comparable or superior performance to larger LLMs, which is shown in Figure \ref{fig:intro_finding}. Moreover, by combining our method with other existing methods, we find the performance can be further improved. By comparing with the performance of complicated methods, the result shows that employing our method solely can achieve comparable performance in most cases. This implies that comparable performance can be achieved without the need for additional handcraft prompt design or complex collaboration frameworks.

\begin{figure}[t!]
    \centering
    \includegraphics[width=0.6\linewidth]{icml2024/fig/intro_finding.png}
...
Additionally, the experimental results indicate a correlation between the efficacy of the performance improvements and the difficulty of the problems addressed. To understand the reasons behind these performance improvements, we analyze the influence of problem difficulty on the effectiveness of our method. We classify difficulty into three dimensions: the inherent difficulty, the number of steps, and the prior probability of the correct answer. Through a series of experiments, we adjust these dimensions and observe their effects independently. %From our results, we conclude three key properties that, when leveraged, can significantly enhance performance using our method.

In this paper, we conduct the first comprehensive study on the phenomenon: the performance of LLMs can be improved by increasing the ensemble size.
Our contributions are summarized as follows:

\begin{itemize}
\item  We propose a simple sampling-and-voting method and demonstrate that the performance of LLMs scales with the number of agents instantiated.
\item We explore the compatibility of our methods with existing methods, revealing that our method can enhance these methods to achieve further performance improvements.
\item We further analyze the effectiveness of our method in tackling problems at varying difficulties, and propose optimization methods.
%\item We conduct the first comprehensive study on the phenomenon: the performance of LLMs can be improved by increasing the ensemble size. We demonstrate that this phenomenon generally exists on various types of datasets across multiple LLMs by employing our method. Additionally, we explore the compatibility of our methods with existing methods, revealing that our method can enhance these methods to achieve further performance improvements.

%\item We further analyze the effectiveness of our method in tackling problems at varying difficulties which are classified into three dimensions: the inherent complexity, the length of reasoning steps, and the probability of a correct response. %we identify three properties and leverage them to further enhance the method's efficacy.
\end{itemize}
