...
\begin{itemize}
    ...
    \begin{align}\label{def:cum-veri-reward}
...
\begin{align*}
    &R_{\btheta}(q, s_{ta}) = \EE \bigg[\frac{1}{\sum_{t' \in [t, T]} \gamma^{t'-t}} \biggl( \text{Verifier}(q, s_{ta})+ \sum_{t' \in [t+1, T]}\sum_{j \in [A]} \frac{1}{A}\gamma^{t'-t}\text{Verifier}(q, s_{t'j})\biggr) \bigg].  
\end{align*}
