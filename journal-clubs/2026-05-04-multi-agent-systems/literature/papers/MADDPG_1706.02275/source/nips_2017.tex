...
\title{Multi-Agent Actor-Critic for Mixed Cooperative-Competitive Environments} %too wordy?
%\title{Actor-Critic Methods for Multi-Agent Environments}
%too general?
%\title{Policy Gradient Methods for\\ Multi-Agent Environments}



%\title{A Multi-Agent Actor-Critic Algorithm for Mixed Cooperative-Competitive Environments}
%\title{An Actor-Critic Algorithm for\\ Multi-Agent Environments}
%\title{A Multi-Agent Actor-Critic Algorithm for Mixed Cooperative-Competitive Environments}
%\title{Multi-Agent Policy Gradient Methods for Mixed Cooperative-Competitive Environments}
%\title{Learning Communication Strategies with\\ Multi-Agent Policy Gradients} % alternate title
%\title{Learning to Communicate with\\ Multi-Agent Policy Gradients} % alternate title 2
% alternate title 3: replace communicate with coordinate
...
% Traditional reinforcement learning approaches are poorly suited to multi-agent environments. In this work, we analyze why traditional algorithms struggle in this setting by deriving the variance of policy gradient as the number of agents grows. 
We then present an adaptation of actor-critic methods that considers action policies of other agents and is able to successfully learn policies that require complex multi-agent coordination. Additionally, we introduce a training regimen utilizing an ensemble of policies for each agent that leads to more robust multi-agent policies. We show the strength of our approach compared to existing methods in cooperative as well as competitive scenarios, where agent populations are able to discover various physical and informational coordination strategies.% such as rudimentary cryptography out of the need to securely communicate.

%We propose a method for learning policies in a model-free, multi-agent reinforcement learning setting. The resulting agents operate in a decentralized manner, using local observations and policies. However, a coordination between agents emerges via our proposed actor-critic approach that uses a decentralized actor and a centralized critic during training. After training has completed, only the resulting decentralized critic policy is necessary to control the agents. We do not place assumptions on the nature of multi-agent interactions and our method supports cooperative, competitive, and mixed scenarios. 

%Such a method explicitly requires modeling behavior of other agents (and we show that learning such models is feasible).

\end{abstract}

\section{Introduction}
%\comment{R: One thing missing from intro: why should we care about mixed cooperative-competitive settings (other than it being `more general')?}
...
%To date, the most impressive RL successes have been in single agent domains (e.g., \cite{mnih2015human}), or in a 2-person game (e.g., \cite{silver2016mastering}). 

%Contemporary methods in reinforcement learning include deep Q-learning \cite{mnih2015human}, policy gradient methods \cite{williams1992simple}, and actor-critic methods that mix both \cite{lillicrap2015continuous,mnih2016asynchronous}. 
...
%Successfully scaling RL to such domains has the potential for RL to play a role in large scale communication and trading systems, and robots interacting with humans, among other applications. 

%However, there are a number of important applications that involve interaction between multiple agents, where emergent behavior and complexity arise from agents co-evolving together. For example, multi-robot control, discovery of communication and language, multi-player games, and the analysis of social dilemmas all operate in a multi-agent domain. Successfully scaling RL to such domains has the potential for RL to play a role in large scale communication and trading systems, and robots interacting with humans, among other applications. 

%\comment{A:this paragraph should probably appear later} Related problems such as hierarchical reinforcement learning can also be seen as a multi-agent system. Additionally, multi-agent self-play has recently been shown as a successful training paradigm~\cite{sukhbaatar2017intrinsic}.

Unfortunately, traditional reinforcement learning approaches such as Q-Learning or policy gradient are poorly suited to multi-agent environments. One issue is that each agent’s policy is changing as training progresses, and the environment becomes non-stationary from the perspective of any individual agent (in a way that is not explainable by changes in the agent's own policy). This presents learning stability challenges and prevents the straightforward use of past experience replay, which is crucial for stabilizing deep Q-learning. Policy gradient methods, on the other hand, usually exhibit very high variance when coordination of multiple agents is required. Alternatively, one can use model-based policy optimization which can learn optimal policies via back-propagation, but this requires a (differentiable) model of the world dynamics and assumptions about the interactions between agents. % R: maybe put motivating hide-and-seek example here?
...
%Further, since these centralized optimization methods optimize a single joint reward they are only applicable in fully-cooperative environments. %%PA: I am not totally sure this last claim is true -- I think it's more subtle: you can run model-based and BPTT procedurally just fine even if not cooperative, but it'll have similar challenges as alluded to earlier about being a bunch of separate optimization problems each faced with a non-stationary environment
...
%to multiple potential strategies of other agents by requiring that agents need to be able to interact well with an ensemble of any possible policies of other agents.
...
\vspace{-2mm}
%Contemporary methods in reinforcement learning include deep Q-learning \cite{mnih2015human}, policy gradient methods \cite{williams1992simple}, and actor-critic methods that mix both \cite{lillicrap2015continuous,mnih2016asynchronous}. However, there are a number of important applications that involve interaction between multiple agents. For example, multi-robot control \cite{matignon12coordinated}, discovery of communication and language \cite{sukhbaatar2016learning,foerster16b,mordatch2017emergence}, multiplayer games \cite{peng17starcraft}, and analysis of social dilemmas \cite{multiagent_ssd} all operate in a multi-agent domain. Related problems, such as variants of hierarchical reinforcement learning \cite{dayan93feudal} can also be seen as a multi-agent system, with multiple levels of hierarchy being equivalent to multiple agents. Additionally, multi-agent self-play has recently been shown as a successful training paradigm \cite{alphago, selfplay17}. 
 %\comment{R: No point repeating this from the introduction -- might as well just add the citations there and cut this paragraph.}
...
%One issue is that each agent’s policy is changing as the training progresses, the environment becomes non-stationary from the perspective of any individual agent, which presents learning stability challenges \cite{matignon12independent} and prevents the use of past experience replay which is crucial for stabilizing deep Q-learning. 
...
%An alternative to model-free methods is model-based policy optimization which can learn optimal policies via centralized learning and back propagation through time \cite{sukhbaatar2016learning,foerster16b,mordatch2017emergence}, but requires a differentiable model of the world dynamics. Such centralized optimization methods that optimize a single joint reward are only applicable in cooperative environments and cannot be applied in competitive settings. \comment{R: change from intro?}

%We propose an approach that is an actor-critic method based on deterministic policy gradients \cite{lillicrap2015continuous}, where the actor only has access to agent’s local observations and actions, but the critic has access to joint observations and actions of all agents during training. After training is completed, only the local actors are use at execution phase, acting in a decentralized manner and is equally applicable in cooperative and competitive settings.  %R: does not belong here
...
%We detail the problem setting and the traditional reinforcement learning (RL) algorithms that can be applied to this setting, as well as issues arising when applying them. 
...
% We consider the finite horizon setting with episodes of length $T$.

%\comment{move to MADDPG section?} As in prior work (e.g.\@ \cite{foerster16a}), we consider the setting of \textit{centralized learning}, where extra information about the policies of other agents may be provided, but \textit{decentralized execution}, where agents must rely only on their local observations to take actions. Unlike Foerster et al.\@ \cite{foerster16a} and Sukhbaatar et al.\@ \cite{sukhbaatar2016learning}, our approach [...]  We also do not assume that the environment dynamics are known and differentiable, as in \cite{mordatch2017emergence}. Our approach is general and can be applied to any task requiring coordination between agents, which may or may not involve explicit communication channels and where agents may be cooperative or adversarial. (emphasize more differences with prior work)
...
%  where $R^t = \sum_{k=t}^\infty \gamma (k-t) r^k$
...
%There is no explicit policy representation: the Q function is represented as a deep neural network which is trained to minimize $\mathcal{L}_i(\theta_{i})$ as defined above for DDPG. To select actions, DQN implements an $\epsilon$-greedy policy, choosing either the action with the highest Q-value (with probability $1-\epsilon$) or a random action (with probability $\epsilon$). DQN makes use of both experience replay and a target network, as with DDPG.

\paragraph{Policy Gradient (PG) Algorithms.} Policy gradient methods are another popular choice for a variety of RL tasks.
%, from continuous control \cite{lillicrap2015continuous} to Atari game-playing \cite{mnih2016asynchronous}.
...
\end{equation}
where $p^{\pol}$ is the state distribution. %In practice, this expectation is approximated by sampling from $p^{\pol}$ and $\pol$.
...
% Consider $N$ agents with binary actions: $P(a_i=1) = \theta_i$, where $R(a_1,\dots,a_N) = \mathbf{1}_{a_1=\dots=a_N}$. We assume an uninformed scenario, in which agents are initialized to $\theta_i=0.5 \ \forall i$. Then, if we are estimating the gradient of the cost $J$ with policy gradient, then the variance of the gradient and the probability of taking a step in the right direction are:
% \begin{align*}
%     \mathbb V (\hat{\dd} J ) = (0.5)^N - (0.5)^{2N} \qquad
%     P(\langle \hat{\nabla} J, \nabla J \rangle > 0) \propto (0.5)^N,
% \end{align*}
% where $\hat{\nabla} J$ is the policy gradient estimator from a single sample, and $\nabla J$ is the true gradient (See Appendix for derivation). Even in such simple cases, the variance of the policy gradient can grow exponentially with the number of agents.
...
%Even in simple cases, we show below that the variance of the policy gradient can grow exponentially with the number of agents.
...
% \textit{Deep deterministic policy gradient} (DDPG) \cite{lillicrap2015continuous} is a variant of DPG where the policy $\cpol$ and critic $Q^{\cpol}$ are approximated with deep neural networks. DDPG is an off-policy algorithm, and samples trajectories from a replay buffer of experiences that are stored throughout training. DDPG also makes use of a target network, as in DQN \cite{mnih2015human}.
%; the Q function is calculated by minimizing the loss $\mathcal{L}_i(\theta) = \mathbb{E}_{s,a,r,s'}[(Q^{\cpol_{\theta}}(s,a) - y_i)^2]$ at each iteration $i$, where $y_i = r + \gamma\max_{a'} Q^{\cpol_{\theta'}}(s', a')$\comment{Yi: remove $\max$?}. The parameters $\theta'$ of the target Q function $Q^{\cpol_{\theta'}}(s', a')$ are the parameters of a network that is frozen for multiple time steps while the parameters $\theta$ are being updated. 

%\comment{non-stationarity for multi-agent}

%Neither standard policy gradient methods nor DQN are particularly suited to multi-agent settings, as they do not directly consider the policies of other agents. We show this in the next section by analyzing the variance of policy gradient algorithms in multi-agent settings, and demonstrate it empirically in Section \ref{sec:experiments}. In Section \ref{sec:methods}, we propose an extension to DDPG that considers the policy of other agents in its Q function during training, but only uses agents' local policy $\pol$ during execution. Note that this is not possible with Q-learning, as the Q function cannot contain different information at training and test time.
...
% \begin{figure}[H]
% \includegraphics[width=1.0\textwidth]{fig/experiment}
% \end{figure}
% \end{minipage} \hfill
% \begin{minipage}{0.70\textwidth}
% \paragraph{Cooperative communication.} This task consists of two cooperative agents, a speaker and a listener, who are placed in an environment with three landmarks of differing colours. At each episode, the listener must navigate to a landmark of a particular colour, and obtains reward based on its distance to the correct landmark. However, while the listener can observe the relative position and color of the landmarks, it does not know which landmark it must navigate to. Conversely, the speaker's observation consists of the correct landmark color, and it can produce a communication output at each time step which is observed by the listener. 
% %This is a similar environment to the one considered in \cite{mordatch2017emergence}, however we consider the case without known and differentiable environment dynamics. 
...
%%PA: include all six in the figure above?  otherwise the remaining 2 might be overlooked?  or if not including all 6, ensure that the 2 which we'd most prefer to be overlooked are the ones not in teh figure; maybe we should just do them all as inline figures (\wrapfigure} with the corresponding text, not sure...
...
% Will add foraging task if there's time
...
\begin{subfigure}{.52\textwidth}
  ...
  %\caption{Reward over training iterations}
\end{subfigure}
\hspace{1mm}
\begin{subfigure}{.48\textwidth}
  \centering
  \includegraphics[width=1\linewidth]{fig/maddpg6}
  %\caption{KL divergence between the approximate policies and the true policies.}
\end{subfigure}
\vspace{-2mm}
...
% To support discrete communication messages, we use the Gumbel-Softmax estimator \cite{jang2016categorical}. 
...
We hypothesize that a primary reason for the failure of traditional RL methods in this (and other) multi-agent settings is the lack of a consistent gradient signal. %Since reward is only obtained based on the actions of the listener, the gradient provided to the speaker can be quite noisy if there is no coordination. 
...
%We show a sample learning curve in Figure \ref{fig:crypto}, showing
...
%Demonstrate problems of traditional RL algorithms in simple communication scenario. The goal for speaker is to send a goal color to the listener who has to go to the right landmark. Show that with episode length 1 all algorithms perform at the level of optimal policy (which is the policy that would have access to goal color). As episode length grows, PG, DQN and DDPG start performing poorly (get stuck in a local minima solution), while MA-DDPG is still able to perform the same as the optimal policy.
...
%For faster convergence, in each episode we randomly choose two integers $k_1,k_2$ uniformly from 1 to $K$ and adopt policies $\cpol_i^{(k_1)}$ for each adversary agent $i$ and $\cpol_j^{(k_2)}$ for each cooperative agent $j$.
%Meanwhile we also train agents in the same environment without policy ensembles.
To evaluate the approach, we measure the performance of ensemble policies and single policies in the roles of both agent and adversary. % of the adversary agents with and without policy ensembles against different cooperative agents. 
...
%if we fix the same competitor and change the other agent from a normal agent to an ensemble agent, the performance will become better. 
...
%%PA: why quadratically? isn't it just linear? as in N * (state-size-of-single-agent + action-size-of-single-agent) ?   [I do realize that maybe multiplicative interactions are needed, and maybe even hardcoded?  but that would be in the Q network, not in the input space]
...
\iffalse
% Shortened algorithm for poster
\begin{algorithm}[H]
 \SetAlgoLined
  \begin{algorithmic}
    \FOR{$\textrm{episode}=1\textrm{ to }M$}
      \FOR{$t=1\textrm{ to max-episode-length}$}
          ...
          \STATE Store $(\mathbf{x},a,r,\mathbf{x}')$ in replay buffer $\mathcal{D}$ %, $\mathbf{x}\gets\mathbf{x}'$
          \FOR{agent $i=1\textrm{ to }N$}
            \STATE Sample a random minibatch of $S$ samples $(\mathbf{x}^j,a^j,r^j,\mathbf{x}'^j)$ from $\mathcal{D}$
            \STATE Set $y^j=r_i^j+\gamma\, Q_i^{\cpol'}(\mathbf{x}'^j,a_1',\ldots,a_N')|_{a_k'=\cpol'_k(o_k^j)}$
            \STATE Update critic by minimizing the loss $\mathcal{L}(\theta_i)=\frac{1}{S}\sum_j\left(y^j-Q_i^{\cpol}(\mathbf{x}^j,a^j_1,\ldots,a^j_N)\right)^2$
...
%\comment{Yi: Ryan, what's the default setting for replay? Is it synced or not?? In my approximate policy experiments I use the synced replay but in the ensemble experiments I didn't since it is impossible to sync.} \comment{Ryan: in the end, I found sync actually didn't give an advantage (or made MADDPG worse), so I re-ran everything without it.}
...
%MADDPG & No & \textbf{84.0\%} & \textbf{0.133}  \\
%DDPG & Yes &  88.4\% & 75.4\% & 13.0\% \\
...
%story here: reward becomes very sparse as number of agents grows (and thus it's hard to find directions of improvement. Q-learning has the same problem and in addition has the problem with using replay buffers in a non-stationary setting. Centralized methods work well, but so far relied on a model (fully differentiable, or differentiable communication). We can do the best of both worlds - train in a fully model-free way in a centralized manner, but end up with a policy that can be executed in a decentralized manner.


% \comment{We should comment on the generality of the above example -- does it apply to DPG? Would also be nice if we had some plots to show this experimentally}

% In Figure \ref{fig:simple_result} we empirically demonstrate the consequence of the result in Proposition \ref{prop:pg}. We plot the number of samples required to reach a performance $J=0.95$ when learning using REINFORCE. Note the exponential increase in required training samples as the number of agents is increased.
% \comment{I will make this figure a bit nicer.}
% \begin{figure}[ht]
% \includegraphics[width=0.5\textwidth]{sample_requirements.png}
% \caption{Number of samples required to reach $J=0.95$ in the Bernoulli agent example.}
% \label{fig:simple_result}
% \end{figure}

% \subsection{Empirical Analysis in a Communicative Environment}

% Add analysis graphs showing why policy gradient methods fail on simple\_speaker\_listener (Jean)
