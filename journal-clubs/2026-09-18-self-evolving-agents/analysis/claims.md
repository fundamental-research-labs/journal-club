...
- **Claim / type:** Retained experience helps recurring FinEvo procedures relative to paired resets. Expert guidance supplies much of the improvement, while skill-only evolution achieves the highest score in one scaffold's carrier comparison with lower token use than full evolution. Reported finding; net practical value unresolved.
- **Support:** `2026-finevo-bench`, [v1 §§4.1–4.4, Tables 3/5](https://arxiv.org/html/2608.06144v1): 120 tasks, 20 scenes, three globally interleaved orders; common Qwen3.7-Max backbone. Four scaffold gains: 9.33–19.37 rubric points. Claude Code carrier comparison: reset 71.58, static expert skill 86.67, full evolution 89.47, memory-only 90.42, skill-only 93.71. Expert/full/skill-only execution-plus-reflection totals are 159,200/765,000/615,600 tokens per task; skill-only exceeds expert guidance by 7.04 rubric points.
- **Documented example:** Appendix C.1–C.5, Tables 25–27 and Figure 10: Task 04's report gives 40.02% gross margin where its source figures give approximately 41.00%. Judge feedback leads to added memory/skill instructions for recomputing the percentage and checking summary figures against detailed calculations. This documents an instruction change; no isolated later-task benefit of that particular check is established. The thesis uses it to explain what could be reused, not as another efficacy result.
- **Challenge / comparability:** Main effects lack across-run intervals. Repeated tasks/orders are not 360 independent task types. Scoring and feedback share rubrics; one expert calibrates 120 outputs. Skill ablation is one scaffold. These are alternative systems, not an expert-initialized learning test. Costs exclude the judge and do not price expert-skill preparation; token counts are not dollar costs. Most adaptive token use is recurring reflection, so additional reuse alone does not establish payback. Rubric points are not accuracy percentage points or user value.
...
- **Challenge / comparability:** The 45 cells share tasks; they are not independent datasets. The reported ± reproduces sample SD of three seed-level means, not a CI. Native benchmark metrics differ. FinEvo also interleaves tasks; its larger gains use different tasks, methods, and rubric feedback, so they are not a direct contradiction, ranking, or mixed-versus-unmixed comparison. Recurrence and task mixing are separate dimensions. The context-integrated versus retrieval-based comparison changes several method components; interference is the authors’ interpretation, not a retrieval-only causal ablation.
...
- **Support:** C003–C010, with primary links and locators above. Positive matched controls and negative stress tests jointly explain the preferred thesis. C005 supplies an explicit quality–token tradeoff and a case where restricting updates improves the observed outcome. It does not establish that periodic updating is better. The thesis-driven coverage pass adds C013–C019 and the original 56 source families, now supplemented by four bounded mechanism/foundation reviews; these are not independent replications. ReasoningBank and bounded internalization gains qualify a simple learning-versus-compute dichotomy.
- **Challenge / comparability:** Extra computation and supplied static expertise may account for gains attributed to updating; specialization narrows transfer rather than disproving learning. Computation and retained experience can also complement each other. Benchmark construction, task difficulty, feedback access, methods, and starting expertise could explain cross-study differences instead of task recurrence or memory selection. FinEvo and AgentStream both interleave tasks. No pooled effect or estimate of which bottleneck dominates is defensible. More reuse does not guarantee an economic return if ongoing updating and operation consume more value than the improved outcomes create. A payoff claim needs a workload horizon and a value assigned to outcomes, not just rubric gains. Study families and shared benchmarks prevent treating every paper as independent confirmation.
...
- **Supporting-only specific-check test (removed from the conclusion after reader feedback):** Give otherwise identical agents the same new company reports with and without the added gross-margin check; compare calculation errors while holding other instructions, tools, model, and resources fixed. This tests whether that instruction helps in the selected setting, not whether the whole learning system is economical. It is proposed analysis, not a FinEvo experiment.
- **Selective-update hypothesis and test:** Compare continuous updating with periods of frozen reuse followed by revision triggered by predefined feedback or environment-change signals. Keep model, initial guidance, task stream, feedback access, and total budget comparable; count monitoring, update, inference, and evaluation resources and measure errors throughout. Better quality per resource or lower cost at matched quality would support selective updating; undetected deterioration or lost adaptation gains would weaken it. FinEvo motivates considering a restricted update design but does not test this temporal schedule. A frozen deployment period is distinct from a frozen diagnostic copy used only to measure retention.
...
| From completing tasks to getting better at the work | C001/C003/C013 explain learning in the whole agent with fixed weights; C007–C008/C016/C020 explain selection, correction, and retention; C009/C021 bound learned adaptation; C011–C012 distinguish improvement from practical payoff. |
