...
## Thesis-driven omission audit — September 16, 2026

**Request:** improve the research skill and execute a new field pass informed by the existing thesis, expecting approximately 50 relevant references. **Starting corpus:** 151 records, 49 shortlist/reviewed-reserve families, 48 notes, ten reading priorities; only 13 distinct primary sources appeared in the thesis. **Cutoff:** September 16, unchanged. This run broadened conceptual/citation coverage rather than treating the recent-paper shortlist as the whole evidence base.

### Scope, queries and actual delegation

The research skill now explicitly separates discovery records, reviewed families, actual thesis use and reading priorities. Three `gpt-5.6-luna` low-reasoning discovery lanes each received a bounded roughly eight-query/two-to-three-seed budget: memory/skills, meta-search/weights, and evaluation/practitioners. Two `gpt-5.6-sol` medium-reasoning lanes screened primary additions and reviewed the expanded argument. Follow-ups read four consequential foundations, rather than stopping automatically at 50. Workers did not recursively delegate; coordinator alone merged records and revised the thesis.

- [Memory queries, selected citation edges and limitations](workers/thesis-memory-coverage.md): followed by primary ExpeL/ReasoningBank reading.
- [Meta-search queries and selected seed edges](workers/thesis-meta-coverage.md): followed by primary continual-internalization reading; stopped at budget, not saturation.
- [Evaluation/practitioner queries and limitations](workers/thesis-evaluation-coverage.md): followed by ADAS/AFlow reading. Original worker candidate JSONs are discovery/provisional records; they are not authoritative metadata.
- [Independent primary screening and version corrections](workers/thesis-additions-screening.md); [expanded-draft critique](workers/thesis-review.md).

Coordinator keyword queries were `self evolving agents self improvement September 2026 continual learning benchmark transfer` and `self improving agents generalization fixed compute strong baselines SkillsBench ADAS 2026`. Primary follow-up opened the exact arXiv HTML versions of MemSkill v2, SkillsBench v4, and continual internalization v1; methods/related-work/results reading supplied decisive checks. ExpeL v3, ReasoningBank v2, ADAS v2 and AFlow v4 metadata and primary mechanisms were then reopened. Reused previous substantive notes and the verified September frontier for the other sources; not every old URL or latest version was independently refreshed.

### What changed and what was rejected

New canonical records: **ExpeL (2023)** and **Rethinking Continual Experience Internalization (June 2026)**. Promotions with new notes: **ADAS, AFlow, ReasoningBank, MemSkill and SkillsBench**. The current register has **153 records: ten shortlist, 46 reviewed reserves, 21 screened reserves, 63 watchlist, three companions and ten exclusions**. There are **55 notes**, one covering two distinct integrity studies. The thesis uses **56 distinct reviewed references**, with near-claim citations and a [complete role map](thesis-coverage.md). These are not independent replications. The ten reading priorities remain unchanged; new comparators are mandatory conceptual companions in the broader thesis.

The thesis now covers memory operations, cross-task foundations, workflow search, environment/curriculum generation, hybrid updates, lifecycle/feedback failures, original practitioner designs, and artifact-versus-improver distinctions. It preserves the bounded-learning interpretation while strengthening two qualifications: memory and extra compute can be complementary (ReasoningBank), and a fixed engineered recipe can sustain several update cycles (continual internalization). Claims C001–C012 remain stable; C013–C019 track the expansion.

Screening rejected an obsolete SkillsBench snapshot (84-task/24.3→40.6 statistics) in favor of current v4 (87 tasks, 18 configurations, 33.9→50.5, task-filtering/authoring caveats). Corrected overbroad MemSkill transfer, internalization's unmeasured third-cycle in-context result, stale duplicate EvoX/MLEvolve dates, AHE's v4 citation and timeout-excluded token mean, the Escher-Loop static-pool tie/slight best-score advantage, and Dream-RSI's mixed math outcomes. Shared notes and source records supersede provisional worker files.

Ancillary reward-seeker, METR, STATE-Bench, cost-analysis and vendor-memory leads remain documented in worker reports without being promoted into thesis evidence. Their additional reading is not necessary to support the current scoped argument, and several require fuller metadata/control checks. X/talk access remains limited; no inaccessible thread or unwatched talk became evidence. Wider multi-agent topology, non-LLM meta-learning foundations, embodied diversity, and independent production replications remain incomplete. The run stops after consequential gaps are addressed within bounded lanes, **not** at claimed search saturation or complete field recall.

### Acquisition, checks and handoff

Retained four CC BY 4.0 originals: MemSkill v2, SkillsBench v4, ExpeL v3 and ADAS v2. ExpeL urllib returned 406; curl retrieved the same official version successfully. AFlow, ReasoningBank and internalization remain linked rather than retained under arXiv's non-exclusive license. Four new PDFs passed title/version text checks and `pdfinfo`; all 43 retained originals matched manifest byte counts and SHA-256. No experiments or newly generated visual artifacts were produced.

Verification passed for 153 unique keys/register anchors, 56 traceable thesis references, 55 notes, ten priorities, seven new note layouts and 525 authored relative links in the affected shared materials. Both changed skills validate using the existing validator in an isolated `uv --with pyyaml` environment; the default Python lacks PyYAML. Repository symlinks make the skill corrections active. Whitespace validation passed after removing a trailing blank line.

Concurrent presentation work appeared during this research pass. Its files and staged changes were preserved. The existing deck/storyboard uses the prior C001–C012 thesis; presentation coverage is therefore pending reconciliation with C013–C019, especially static skills, foundations, multi-cycle evidence and mixed meta-search outcomes. Rebuilding it is outside this research request. The workflow and README identify this downstream handoff.

## September 18, 2026 — targeted scholarly-foundations repair

Question: which established continual/lifelong-learning and meta-learning concepts are missing from the thesis, and how do they change its novelty and evaluation claims? Existing source register/originals lacked the four foundations added here. Searched primary CMU, NeurIPS, PNAS and PMLR material; opened Thrun's November 1995 report, GEM proceedings PDF, EWC arXiv v2, and MAML proceedings PDF. Exact reading scopes and versions are in their notes and source records. PNAS DOI full-text access failed; EWC's author PDF supplied the method, with publication identity checked separately. An initial “Learning One More Thing” lead was not used as evidence; Thrun's accessible report provides the scoped foundation.

Read relevant full-text sections before writing; no abstract-only findings entered the thesis. Reopened local SEAL v2 for its explicit meta-learning and forgetting statements. No broad freshness rerun or training experiment: frontier cutoff remains September 16. Stopped after grounding transfer, retention, update mechanisms and the meta-learning boundary; broader historical survey and citation-network expansion remain outside this repair. See [coverage reconciliation](thesis-coverage.md#scholarly-foundations-revision--september-18-2026).
